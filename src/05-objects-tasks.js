...
function fromJSON(proto, json) {
  ...
  Object.assign(c, JSON.parse(json));
  return c;
}
...
class Selector {
  ...
  getLastSelector() {
    return this.result.length && this.result.slice(-1)[0];
  }

  validateOrder(type) {
    const last = this.getLastSelector();
    if (last && this.SELECTORS_ORDER[last.type] > this.SELECTORS_ORDER[type]) {
      throw new Error('Selector parts should be arranged in the following order: element, id, class, attribute, pseudo-class, pseudo-element');
    }
  }

  validateUniqueness(type) {
    if (this.result.some((el) => el.type === type)) {
  ...
  stringify() {
    return this.result.map((el) => el.value).join('');
  }
}
